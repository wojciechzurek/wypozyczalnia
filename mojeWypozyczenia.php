		...
		<div id="wypozyczalniaMain">
			...
			<?php
				if ($succ_login)
				{
					...
					$selekcik = $db->prepare("SELECT w.id AS idx, p.obrazek, p.nazwa, w.type, w.date_from, w.date_to FROM wypozyczenia AS w INNER JOIN przedmioty AS p ON w.itemID = p.id WHERE w.userID = :userID ORDER BY w.date_from;");
